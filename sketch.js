...
function handlePlayerArrowCollision() {
  for (var i = 0; i < playerArrows.length; i++) {
    var basecollided = Matter.SAT.collides(
      playerArrows[i].body,
      computerBase.body
    );

    var archercollided = Matter.SAT.collides(
      playerArrows[i].body,
      computerArcher.body
    );

    var computercollided = Matter.SAT.collides(
      playerArrows[i].body,
      computer.body
    );

    if (
      basecollided.collided ||
      archercollided.collided ||
      computercollided.collided
    ) {
